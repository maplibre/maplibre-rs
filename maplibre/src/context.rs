...
impl ViewState {
    ...
    pub fn update_zoom(&mut self, new_zoom: Zoom) -> Zoom {
        let mut clamped_new_zoom = new_zoom;
        clamped_new_zoom.clamp(self.min_zoom as f64, self.max_zoom as f64);

        *self.zoom = clamped_new_zoom;
        log::info!("zoom: {}", clamped_new_zoom);
